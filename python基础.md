...
#### python多线程爬虫 参见：https://zhuanlan.zhihu.com/p/35944711  
原理主要是将爬取，解析分离，分别建立线程组，并通过队列获取数据。 我觉得可以增加一个线程，专门负责写入文件，并保证写入的顺序，具体方法是：解析后的内容存入一个字典中，写入线程按顺序循环去查询，如果获取到了，将内容写入文件，并且在字典中删除该内容。
